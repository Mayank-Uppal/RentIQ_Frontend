...
function HomeModal({ handlesubmit, propertyId }: modalProps) {
    ...
    const onsubmit=async(e:React.FormEvent) => {
        ...
        axios.post(
        `https://rentiq-project.onrender.com/property/${propertyId}/invite`, 
        emails, 
        { 
            headers: { 
    ...
    .then((res) => console.log("Invite sent:", res.data))
...
function HomeModal({ handlesubmit, propertyId }: modalProps) {
    ...
    // ✅ Reset only isSending when modal opens, DON'T clear emails
    useEffect(() => {
        setIsSending(false);
    }, [propertyId]);
    ...
    const onsubmit = async (e: React.FormEvent) => {
        ...
        console.log("2. isSending:", isSending);
        console.log("3. emails:", emails);
        ...
        try {
            ...
            setIsSending(false);
            setemail([]); // ✅ Clear emails AFTER sending (in finally block)
        }
    }
                ...
                <div className="relative p-4 w-full max-w-md max-h-full">
                    <div className="relative bg-neutral-primary-soft border border-default rounded-base shadow-sm p-4 md:p-6">
                        <div className="flex items-center justify-between border-b border-default pb-4 md:pb-5">
                            ...
                            <button type="button" className="text-body bg-transparent hover:bg-neutral-tertiary hover:text-heading rounded-base text-sm w-9 h-9 ms-auto inline-flex justify-center items-center" data-modal-hide="authentication-modal">
                                <svg onClick={()=>handlesubmit(false)} className="w-5 h-5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24"><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" strokeWidth="2" d="M6 18 17.94 6M18 18 6.06 6" /></svg>
                                <span className="sr-only">Close modal</span>
                            </button>
                        </div>
                        <form onSubmit={onsubmit} className="pt-4 md:pt-6">
                            <div className="mb-4">
                                ...
                                <input 
                                    type="email" 
                                    id="email" 
                                    className="bg-neutral-secondary-medium border border-default-medium text-heading text-sm rounded-xs focus:ring-brand focus:border-brand block w-full px-3 py-2.5 shadow-xs placeholder:text-body" 
                            ...
                            <div className="flex md:flex-row flex-col gap-8 py-2">
                                <button 
                                    ...
                                    type="button" 
                                    className="text-white border-indigo-500 btn btn-outline bg-black hover:bg-indigo-500 px-12 py-6"
                                >
                                    Add Email
                                </button>
                                <button 
                                    type="submit" 
                                    disabled={isSending}
                                    className={`text-white hover:border-indigo-500 btn btn-outline bg-indigo-500 hover:bg-black px-12 py-6 ${isSending ? 'opacity-50 cursor-not-allowed' : ''}`}
                                >
                        ...
                        <ul className="py-3">
                            {emails.map((email, index) => (
                                <div key={index} className="flex items-center w-full max-w-sm p-4 mb-2 text-body bg-neutral-primary-soft rounded-base shadow-xs border border-default" role="alert">
                                    <div className="inline-flex items-center justify-center shrink-0 w-7 h-7 text-fg-success bg-success-soft rounded">
                                    ...
                                    <div className="ms-3 text-sm font-normal">{email}</div>
                                    <button type="button" className="ms-auto flex items-center justify-center text-body hover:text-heading bg-transparent box-border border border-transparent hover:bg-neutral-secondary-medium focus:ring-4 focus:ring-neutral-tertiary font-medium leading-5 rounded text-sm h-8 w-8 focus:outline-none" aria-label="Close">
                                        <span className="sr-only">Close</span>
                                        <svg onClick={() => handleRemoveEmail(email)} className="w-5 h-5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
